<script lang="ts">
	...
	import EmailButton from "./data-table-rtl/email-button.svelte";
	import Actions from "./data-table-rtl/actions.svelte";
	...
	import {
		...
		renderComponent,
		renderSnippet,
	} from "@/svelte-ui/data-table/index.js";
	...
	const columns: ColumnDef<Payment>[] = [
		...
		},
		{
			...
			header: () =>
				renderSnippet(
					createRawSnippet(() => ({
						render: () => `<div class="text-end">المبلغ</div>`,
					}))
				),
			cell: ({ row }) => {
				...
				const amountCellSnippet = createRawSnippet<[{ amount: number }]>(
					(getAmount) => {
						...
						return {
							render: () => `<div class="text-start font-medium">${formatted}</div>`,
						};
					}
				);
		...
		},
		{
			id: "actions",
			enableHiding: false,
			cell: ({ row }) => renderComponent(Actions, { id: row.original.id }),
		},
	];
			...
			<DropdownMenu.Trigger>
				{#snippet child({ props })}
					<Button {...props} variant="outline" class="ms-auto">
						الأعمدة <ChevronDownIcon class="ms-2 size-4" />
					</Button>
				{/snippet}
			</DropdownMenu.Trigger>
