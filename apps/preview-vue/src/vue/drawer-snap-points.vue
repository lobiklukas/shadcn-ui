...
const activeSnap = ref<number | string | null>(snapPoints[0])
</script>

<template>
  <Drawer v-model:active-snap-point="activeSnap" :snap-points="snapPoints">
    ...
    <DrawerContent class="h-[85vh]">
      <DrawerHeader>
        ...
        <DrawerDescription
          >Drag the drawer between three heights.</DrawerDescription
        >
      </DrawerHeader>
      <div
      ...
      >
        <p>
          Active snap point: {{ snapPoints.indexOf(Number(activeSnap)) + 1 }} /
          {{ snapPoints.length }}
        </p>
