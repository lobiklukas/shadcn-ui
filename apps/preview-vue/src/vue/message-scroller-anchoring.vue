...
const scripted = [
  ...
  [
    "assistant",
    "Done — each of your messages settles near the top as the transcript grows.",
  ],
  ["user", "Now switch anchoring to your replies."],
  [
    "assistant",
    "Switching the anchor role moves the settled edge to assistant turns.",
  ],
  ["user", "Add a few more lines so I can see it."],
  [
    "assistant",
    "Sure — here are several more lines to make the viewport scroll and demonstrate the anchored edge behavior clearly.",
  ],
] as const

function addNext() {
  if (nextId >= scripted.length) return
...
<template>
  <Card class="mx-auto flex h-[28rem] w-full max-w-sm flex-col gap-0">
    <CardHeader class="border-b">
      ...
      <CardDescription
        >Choose which role settles near the top edge.</CardDescription
      >
      <CardAction>
        <Button
          variant="outline"
          ...
          <MessageScrollerViewport>
            <MessageScrollerContent class="gap-3 p-4">
              <MessageScrollerItem
                v-for="m in messages"
                :key="m.id"
                :scroll-anchor="m.role === anchorRole"
              >
                <Bubble
                  :align="m.role === 'user' ? 'end' : 'start'"
                  :variant="m.role === 'user' ? 'tinted' : 'outline'"
                >
                  <BubbleContent>
