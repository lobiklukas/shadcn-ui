import { Component } from "@angular/core"

import { Label } from "@/angular-ui/label"
import { InputGroup, InputGroupAddon, InputGroupInput } from "@/angular-ui/input-group"

// React example uses ButtonGroup/ButtonGroupText; button-group is not yet
// ported to Angular, so its layout is inlined (flex row + muted bordered text
// slots matching cn-button-group-text).
@Component({
  selector: "preview-input-group-button-group",
  standalone: true,
  imports: [Label, InputGroup, InputGroupAddon, InputGroupInput],
  template: `
    <div class="grid w-full max-w-sm gap-6">
      <div class="flex w-full items-center">
        <label
          uiLabel
          for="url"
          class="flex items-center rounded-l-lg border border-r-0 bg-muted px-2.5 py-1 text-sm font-medium shadow-xs"
          >https://</label
        >
        <div uiInputGroup class="min-w-0 flex-1 rounded-none">
          <input uiInputGroupInput id="url" />
          <div uiInputGroupAddon align="inline-end">
            <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960">
              <path
                d="M180-120q-24 0-42-18t-18-42v-600q0-24 18-42t42-18h249q12.75 0 21.38 8.68 8.62 8.67 8.62 21.5 0 12.82-8.62 21.32-8.63 8.5-21.38 8.5H180v600h600v-249q0-12.75 8.68-21.38 8.67-8.62 21.5-8.62 12.82 0 21.32 8.62 8.5 8.63 8.5 21.38v249q0 24-18 42t-42 18H180Zm600-617L403-360q-9 9-21 8.5t-21-9.5q-9-9-9-21t9-21l377-377H549q-12.75 0-21.37-8.68-8.63-8.67-8.63-21.5 0-12.82 8.63-21.32 8.62-8.5 21.37-8.5h261q12.75 0 21.38 8.62Q840-822.75 840-810v261q0 12.75-8.68 21.37-8.67 8.63-21.5 8.63-12.82 0-21.32-8.63-8.5-8.62-8.5-21.37v-188Z"
              />
            </svg>
          </div>
        </div>
        <span
          class="-ml-px flex items-center gap-2 rounded-r-lg border bg-muted px-2.5 py-1 text-sm font-medium shadow-xs [&_svg:not([class*='size-'])]:size-4"
          >.com</span
        >
      </div>
    </div>
  `,
})
export class InputGroupButtonGroupComponent {}

export default InputGroupButtonGroupComponent
