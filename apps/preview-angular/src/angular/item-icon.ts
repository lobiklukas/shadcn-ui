import { Button } from "@/angular-ui/button"
import {
  Item,
  ItemActions,
  ItemContent,
  ItemDescription,
  ItemMedia,
  ItemTitle
} from "@/angular-ui/item"
import { Component } from "@angular/core"

@Component({
  selector: "preview-item-icon",
  standalone: true,
  imports: [Item, ItemMedia, ItemContent, ItemTitle, ItemDescription, ItemActions, Button],
  template: `<div class="flex w-full max-w-lg flex-col gap-6">
    <div uiItem variant="outline">
      <div uiItemMedia variant="icon">
        <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960"><path d="M504-344q10-10 10-24t-10-24q-10-10-24-10t-24 10q-10 10-10 24t10 24q10 10 24 10t24-10Zm-2.5-132.63q8.5-8.62 8.5-21.37v-147q0-12.75-8.68-21.38-8.67-8.62-21.5-8.62-12.82 0-21.32 8.62-8.5 8.63-8.5 21.38v147q0 12.75 8.68 21.37 8.67 8.63 21.5 8.63 12.82 0 21.32-8.63ZM470.12-85q-4.56-1-9.12-3-139-47-220-168.5t-81-266.61V-719q0-19.26 10.88-34.66Q181.75-769.07 199-776l260-97q11-4 21-4t21 4l260 97q17.25 6.93 28.13 22.34Q800-738.26 800-719v195.89Q800-378 719-256.5T499-88q-4.56 2-9.12 3T480-84q-5.32 0-9.88-1Zm9.88-58q115-38 187.5-143.5T740-523v-196l-260-98-260 98v196q0 131 72.5 236.5T480-143Zm0-337Z"/></svg>
      </div>
      <div uiItemContent>
        <div uiItemTitle>Security Alert</div>
        <p uiItemDescription>New login detected from unknown device.</p>
      </div>
      <div uiItemActions>
        <button uiButton size="sm" variant="outline">Review</button>
      </div>
    </div>
  </div>`
})
export class ItemIconComponent {}

export default ItemIconComponent
