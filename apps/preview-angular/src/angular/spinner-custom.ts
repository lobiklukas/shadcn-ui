import { Component } from "@angular/core"

// Custom spinner: any spinning glyph works — here Material Symbols'
// progress_activity with animate-spin, matching the React custom example.
@Component({
  selector: "preview-spinner-custom",
  standalone: true,
  template: `<div class="flex items-center gap-4">
    <svg
      role="status"
      aria-label="Loading"
      class="size-4 animate-spin"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 -960 960 960"
    >
      <path d="M323-111q-73-31-127-85t-85-127q-31-73-31-157t31-157q31-73 85-127t127-85q73-31 157-31 12 0 21 9t9 21q0 12-9 21t-21 9q-141 0-240.5 99.5T140-480q0 141 99.5 240.5T480-140q141 0 240.5-99.5T820-480q0-12 9-21t21-9q12 0 21 9t9 21q0 84-31 157t-85 127q-54 54-127 85T480-80q-84 0-157-31Z" />
    </svg>
  </div>`,
})
export class SpinnerCustomComponent {}

export default SpinnerCustomComponent
