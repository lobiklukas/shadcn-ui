...
// Chart uses unovis (the Vue chart stack) instead of recharts.
  ...
  <Card class="@container/card">
    <CardHeader>
      ...
      <CardAction>
        <ToggleGroup
          ...
          @update:model-value="(v: unknown) => timeRange = typeof v === 'string' ? v : timeRange"
        >
        ...
        <Select
          :model-value="timeRange"
          @update:model-value="(v) => { if (typeof v === 'string') timeRange = v }"
        >
          <SelectTrigger
    ...
    <CardContent class="px-2 pt-4 sm:px-6 sm:pt-6">
      <ChartContainer :config="chartConfig" class="aspect-auto h-[250px] w-full">
        <VisXYContainer :data="filteredData" class="h-full w-full">
          <VisArea
