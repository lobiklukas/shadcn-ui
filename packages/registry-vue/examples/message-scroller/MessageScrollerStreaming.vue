...
<template>
  <Card class="mx-auto flex h-[28rem] w-full max-w-sm flex-col gap-0">
    <CardHeader class="border-b">
      ...
      <CardAction>
        <Button variant="outline" size="icon" aria-label="Reset stream" @click="reset">
          <RotateCwIcon />
        </Button>
      </CardAction>
    </CardHeader>
        ...
        <MessageScroller class="h-full">
          <MessageScrollerViewport>
            <MessageScrollerContent class="gap-2 p-4">
              <MessageScrollerItem v-for="m in messages" :key="m.id" :scroll-anchor="true">
                <span
                  class="inline-block max-w-[85%] whitespace-pre-wrap rounded-lg px-3 py-2 text-sm"
                  :class="m.role === 'user' ? 'ml-auto bg-primary text-primary-foreground' : 'bg-muted'"
                >{{ m.text }}</span>
              </MessageScrollerItem>
            </MessageScrollerContent>
          </MessageScrollerViewport>
