...
<template>
  <Card class="mx-auto flex h-[28rem] w-full max-w-sm flex-col gap-0">
    <CardHeader class="border-b">
      ...
      <CardDescription>Choose how new messages enter the conversation.</CardDescription>
      <CardAction>
        <Button variant="outline" size="icon" aria-label="Reset animated messages" :disabled="messages.length === 0" @click="reset">
          <RotateCwIcon />
        </Button>
      </CardAction>
    </CardHeader>
        ...
        <MessageScroller class="h-full">
          <MessageScrollerViewport>
            <MessageScrollerContent class="gap-2 p-4">
              <MessageScrollerItem v-for="m in messages" :key="m.id" :scroll-anchor="m.role === 'user'">
                <span
                  class="inline-block max-w-[85%] rounded-lg px-3 py-2 text-sm transition-all duration-300"
                  :class="[presets[presetId], m.role === 'user' ? 'ml-auto bg-primary text-primary-foreground' : 'bg-muted']"
                >{{ m.text }}</span>
              </MessageScrollerItem>
            </MessageScrollerContent>
          </MessageScrollerViewport>
    ...
    <div class="flex items-center gap-2 border-t p-4">
      <select v-model="presetId" class="rounded-md border bg-transparent px-2 py-1 text-sm" aria-label="Animation preset">
