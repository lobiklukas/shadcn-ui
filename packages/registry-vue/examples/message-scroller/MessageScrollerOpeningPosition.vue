...
const ScrollOnMount = defineComponent({
  props: { position: { type: String, required: true }, positionKey: { type: Number, required: true } },
  setup(innerProps) {
        ...
        <MessageScroller class="h-full">
          <MessageScrollerViewport>
            <MessageScrollerContent class="gap-2 p-4">
              <MessageScrollerItem v-for="m in messages" :key="m.id" class="text-sm text-muted-foreground">
                {{ m.text }}
              </MessageScrollerItem>
            </MessageScrollerContent>
          </MessageScrollerViewport>
