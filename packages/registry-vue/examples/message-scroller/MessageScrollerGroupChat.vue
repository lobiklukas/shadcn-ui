...
interface Turn { id: number; sender: string; role: 'me' | 'them'; text: string }
...
const initial: Turn[] = [
  turn('Grace', 'them', '@mary, the astrophage line keeps matching Venus energy output. Can you check my math?'),
  turn('Mary (Agent)', 'them', 'Checked — your exponent is off by one. The curve should be E ∝ L² not E ∝ L.'),
...
<template>
  <Card class="mx-auto flex h-[28rem] w-full max-w-sm flex-col gap-0">
    <CardHeader class="border-b">
      ...
      <CardAction>
        <Button variant="outline" size="icon" aria-label="Reset chat" @click="messages = [...initial]">
          <RotateCwIcon />
        </Button>
      </CardAction>
    </CardHeader>
        ...
        <MessageScroller class="h-full">
          <MessageScrollerViewport>
            <MessageScrollerContent class="gap-3 p-4">
              <MessageScrollerItem v-for="m in messages" :key="m.id" :scroll-anchor="m.role === 'me'">
                <p class="mb-1 text-xs font-medium text-muted-foreground">{{ m.sender }}</p>
                <Bubble :align="m.role === 'me' ? 'end' : 'start'" :variant="m.role === 'me' ? 'tinted' : 'outline'">
                  <BubbleContent>
