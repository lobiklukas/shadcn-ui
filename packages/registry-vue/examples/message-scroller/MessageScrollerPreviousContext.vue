...
const previousContext: Msg[] = [
  { id: -2, role: 'user', text: 'Earlier today we discussed the migration plan.' },
  { id: -1, role: 'assistant', text: 'Right — phase one ships behind a flag, phase two flips the default.' },
]
...
const messages = computed(() => (showContext.value ? [...previousContext, ...current] : current))
</script>
...
<template>
  <Card class="mx-auto flex h-[28rem] w-full max-w-sm flex-col gap-0">
    <CardHeader class="border-b">
      ...
      <CardDescription>Bring the earlier conversation back above the live one.</CardDescription>
    </CardHeader>
