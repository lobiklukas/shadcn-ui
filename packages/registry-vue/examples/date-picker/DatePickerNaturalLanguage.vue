...
function preset(days: number) {
  nativeDate.value = today(getLocalTimeZone()).add({ days }).toDate(getLocalTimeZone())
}
</script>
...
<template>
  <div class="flex max-w-md flex-col gap-3">
    ...
    <div class="relative flex gap-2">
      <Input id="date" v-model="inputValue" placeholder="Tomorrow or next week" class="bg-background pr-10" />
      <Popover v-model:open="open">
        <PopoverTrigger as-child>
          <Button variant="ghost" class="absolute top-1/2 right-2 size-6 -translate-y-1/2" aria-label="Select date">
    ...
    <div class="flex gap-2 px-1">
      ...
      <Button variant="outline" size="sm" @click="preset(30)">Next month</Button>
    </div>
    <div class="text-muted-foreground px-1 text-sm">
      Your post will be published on
      <span class="font-medium">{{ nativeDate.toLocaleDateString('en-US', { day: '2-digit', month: 'long', year: 'numeric' }) }}</span>.
    </div>
  </div>
</template>
