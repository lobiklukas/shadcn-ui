...
function fmt(value?: DateValue) {
  return value ? value.toDate(getLocalTimeZone()).toLocaleDateString('en-US', { dateStyle: 'medium' }) : ''
}
</script>
    ...
    <Popover v-model:open="open">
      <PopoverTrigger as-child>
        ...
        >
          ...
          {{ dateRange?.start ? (dateRange.end ? `${fmt(dateRange.start)} - ${fmt(dateRange.end)}` : fmt(dateRange.start)) : 'Pick a date' }}
        </Button>
      </PopoverTrigger>
